...
directionAngle <- function(coordX1, coordY1, coordX2, coordY2){
  ...
  #     the subject was heading. The positions are logged by the paradigm each XX ms.
  #     The calculated angle is 0?? when facing right (east) and gets larger while turning counterclockwise 
  #     (North = 90??, west = 180??, south = 360??)
  ...
  #     Walking to the right (east) =>  deltaX > 0
  #     Walking to the left (west)  =>  deltaX < 0
  #     Walking north               =>  deltaY > 0
  #     Walking south               =>  deltaY < 0
  
  #         ### Added "as.numeric" because tables that we want to read only save data in character format!
...
classifyAngle <- function(angle, numberSubclasses){
  ...
  for(i in 1:numberSubclasses){
    ...
    if(angle >= (0 + ((i-1)*360/numberSubclasses)) && angle < (i * (360/numberSubclasses))){
      angleSubclass <- i
...
#datadir = '/gdrive/data/SAM/behavioral/'                               # used to be the directory in Nico's template
datadir = '/Volumes/MPRG-Neurocode/Users/christoph/SAM_behavioral/'     # directory on the server
#datadir = '/Users/koch/Documents/SAM_behavioral/'                      # local directory (in case of missing server connection)

# STILL HAS TO BE REWRITTEN AS BIDS CONFORM!!!!
# Set path for newer BIDS dir:
bidsDir = '/Users/koch/Tardis/direction_decoding_BIDS/'

# Set path for imaging data:
trDir = '/Volumes/MPRG-Neurocode/Data/SAM_nico/'

# find all files in the directory named with 'SAM' (our relevant data)
files = dir(datadir, pattern='SAM')
# split them by run 
...
for (i in 1:length(files1)) {
  ...
  # Exclude those with only 1 run
exclids = unique(setdiff(ids, ids2))
...
# set up vektor for storing number of TRs for each person
nrTRsLogAll = c(NULL)
...
#=====================INNER LOOP: Files=======================================================================================================
  ...
  # append both position vectors and sort them so location and reposition logs have the correct order   
    ...
    # Identify location and reposition logs by adding flag columns filled with 1's (locaion) and 2's (reposition)
	  ...
	  # probably not important for us, because we leave out the transfer phase which would switch the environments
		...
		for(i in 2:length(locRepdata[,1])) {
		  ...
		  angleArray[i,3] <- classifyAngle(angleArray[i,2],6)
	  ...
	  firstPulseidx = which(allPulseidx == thirdPulseidx)
		...
		# Deviding by 2.4 so we get TR number
		trVec_backwards = (trVec_backwards/2.4)
    ...
    # Exclude every row before first scanner pulse:
		...
		#=====================Save data of no movement events for analysis of standing still representations of direction=========================
		
		# Save data with all still standing periods
		standing_data = locRepdata[which(locRepdata[,7] < 0),]
		# Recalculate angle bin based on YAW data
		for(p in 1:nrow(standing_data)){
		  standing_data[p,7] = classifyAngle(standing_data[p,8], 6)
		}
		# Eliminate rows in which not standing still longer than 1s
		# Mark all entries without movement with 1
		diff_vec = matrix(0, nrow(standing_data), 1)
		diff_vec[which(diff(standing_data[,5]) == 0)] = 1
		# Diff always marks one value too few at the end due to difference calculation
		diff_vec[(which(diff(standing_data[,5]) == 0)) + 1] = 1
		# Take only data without movement
		standing_data = standing_data[diff_vec == 1,]
		
		# Exclude events including turning
		# Flag logs in which YAW did not change (rounded to two digits)
		turn_flag = standing_data[,5]
		turn_flag[diff(round(standing_data[,8], 2)) != 0] = 1
		# Add flag to data frame
		standing_data = cbind(standing_data, turn_flag)
		# Select only logs in which participant did not turn
		standing_data = subset(standing_data, turn_flag == 0)

		# Exclude events < 1s
		# Flag time jumps in logs which are higher than sampling rate (+ rounding error)
		event_switch = standing_data[,5]
		event_switch[diff(standing_data[,1]) > 0.2] = 1
		standing_data = cbind(standing_data, event_switch)
		# Get logs in which time jumped
		event_switch_loc = which(standing_data[,13] > 0)
		# Add last entry of log file to get interval with last event
		event_switch_loc = append(event_switch_loc, nrow(standing_data))
		# Keep track of length of events
		exclude_lines = standing_data[,5]
		# Loop over intervals between time jumps
		for(count in 1:length(event_switch_loc)){
		  # (Special case for first interval, has to be 1 because can be compared to other timepoint)
		  if(count == 1){
		    event_lb = 1
		    event_ub = event_switch_loc[count]
		  # (Special case for last interval, has to be 1 because can be compared to other timepoint)
		  } else if(count == length(event_switch_loc)){
		    event_lb = event_switch_loc[(count-1)]
		    event_ub = event_switch_loc[count]
		  }
		  else{
		    event_lb = event_switch_loc[(count-1)]
		  ...
		  # if there are fewer than 1s covered in an interval between two time jumps
		  if((event_ub - event_lb) >= 10){
		    # Number those events which are long enough
		    exclude_lines[seq(event_lb+1, event_ub)] = count
		  }
		}
		# Add exclude lines to data frame
		standing_data = cbind(standing_data,exclude_lines)
		# Keep only logs in which events where > 1s
		standing_data = subset(standing_data, exclude_lines != 0)
		# Adjust event numbers to be without jumps
		for(event_count in c(1:length(unique(standing_data[,14])))){
		  checked_event = unique(standing_data[,14])[event_count]
		  standing_data[,14][standing_data[,14] == checked_event] = event_count
		}
		# Throw away columns and sort to right order
		standing_data = standing_data[,c(1:3,5:6,8,7,10,11,14)]
		
		# Save empty file in case there are no events left
		if(is.null(nrow(standing_data))){
		  standing_data = matrix(NA, 0,10)
		}
		
		# Save standing data
		# Name the columns of the matrix accodringly
		colnames(standing_data) <- c('Time',
		                             'x coordinate',
		                             'y coordinate',
		                             'movement speed',
		...
		if(substr(cfiles[f],4,4) == 1){
		  file = paste(bidsDir,
		               as.character('derivatives/'),
		               as.character('sub-young'),
		               as.character(substr(cfiles[f], 5, 6)),
		               as.character('_3d'),
		               as.character(substr(cfiles[f], nchar(cfiles[f])-4, nchar(cfiles[f])-4)),
		               as.character('_standing'),
		               as.character('.tsv'),
		               sep = '')
		  write.table(standing_data,
		              file = file,
		              sep = '\t',
		              quote = FALSE,
		              col.names = TRUE,
		  ...
		  if(dir.exists(paste(bidsDir,
		    ...
		    dest = paste(bidsDir,
		                 as.character('derivatives/excludes/'),
		                 as.character('sub-young'),
		                 as.character(substr(cfiles[f], 5, 6)),
		                 as.character('/behavior/'),
		                 sep = '')
		...
		for(p in 1:nrow(locRepdata)){
		  locRepdata[p,7] = classifyAngle(locRepdata[p,8], 6)
		}
		...
		angleChangeidx = as.numeric(diff(locRepdata[,7]) != 0)
...
#_____________________________________________________________________________________________________________________________________________
		...
		# Add buffer and event id and write .tsv table with BIDS conform naming
		if(substr(cfiles[f],4,4) == 1){
		  
		  if(dir.exists(paste(bidsDir,as.character('derivatives/'), as.character('sub-young'), 
		                      as.character(substr(cfiles[f], 5, 6)), as.character('/behavior/'), sep = ''))){
  		    
  		  ### --  Added after buffering analysis  -- ###
  		  # Load buffer mask showing which TR was in which buffer
  		  buffer_file = paste(bidsDir,as.character('derivatives/'),
  		                      as.character('sub-young'), as.character(substr(cfiles[f], 5, 6)), '/behavior/',
  		                      as.character('sub-young'), as.character(substr(cfiles[f], 5, 6)), '_buffermask',
  		                      as.character(substr(cfiles[f], nchar(cfiles[f])-4, nchar(cfiles[f])-4)), as.character('.tsv'),  sep = '')
  		  buffer_mask = read.csv(file=buffer_file, header=FALSE, sep='\t')
  
  		  # Add buffer mask to data (only works for locRepdataExcl_10_export) and name them
  		  locRepdataExcl_10_export = cbind(locRepdataExcl_10_export, buffer_mask)
  		  colnames(locRepdataExcl_10_export)[10:12] = c('buffer', 'event in fold', 'fold_id')
		  }
		  ...
		  ### --  Added after buffering analysis  -- ###
		  # Load buffer mask showing which TR was in which buffer
		  if(dir.exists(paste(bidsDir,as.character('derivatives/'), as.character('sub-old'), 
		                      as.character(substr(cfiles[f], 5, 6)), as.character('/behavior/'), sep = ''))){
		    
  		  buffer_file = paste(bidsDir,as.character('derivatives/'),
  		                      as.character('sub-old'), as.character(substr(cfiles[f], 5, 6)), '/behavior/',
  		                      as.character('sub-old'), as.character(substr(cfiles[f], 5, 6)), '_buffermask',
  		                      as.character(substr(cfiles[f], nchar(cfiles[f])-4, nchar(cfiles[f])-4)), as.character('.tsv'),  sep = '')
  		  buffer_mask = read.csv(file=buffer_file, header=FALSE, sep='\t')
		...
		#=====================Delete previous backwards walking data============================
		if(substr(cfiles[f],4,4) == 1){
		  file = paste(bidsDir,as.character('derivatives/'),
		               as.character('sub-young'), as.character(substr(cfiles[f], 5, 6)), as.character('_3d'),
    ...
    # Only export this file if the subject has backwards entries
		if(length(backwards_idx > 0)){
